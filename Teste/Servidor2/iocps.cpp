...
bool inicializar() {
	...
	g_nThreads = WORKER_THREADS_PER_PROCESSOR * getNoProcessadores();
...
DWORD WINAPI AcceptThread(LPVOID lParam) {
	...
	while(WaitForSingleObject(g_hShutdownEvent, 0) != WAIT_OBJECT_0) {
		if ( WSAWaitForMultipleEvents(1, &g_hAcceptEvent, FALSE, WAIT_TIMEOUT_INTERVAL, FALSE) != WSA_WAIT_TIMEOUT ) {
