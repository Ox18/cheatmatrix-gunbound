			...
			__try {
				__try {
					...
					if (aimbot) {
						...
						TextOutA(hdc, sx + 10, sy + 30, "Mobile: ", 8);
						TextOutA(hdc, sx + 10, sy + 45, "Alvo: ", 6);
						TextOutA(hdc, sx + 10, sy + 60, "BackShot: ", 10);
						TextOutA(hdc, sx + 10, sy + 75, "Modo: ", 6);
						TextOutA(hdc, sx + 10, sy + 90, "Efeito: ", 6);
						TextOutA(hdc, sx + 10, sy + 105, "AutoShot: ", 10);
						TextOutA(hdc, sx + 10, sy + 120, "Status: ", 8);
						...
						TextOutA(hdc, sx + 76, sy + 60, (aimbot->backShot ? "Sim" : "Nao"), 3);
						switch((int)aimbot->modoMira) {
						case 0:
							TextOutA(hdc, sx + 50, sy + 75, "Mais Perto", 10);
							break;
						case 1:
							TextOutA(hdc, sx + 50, sy + 75, "Mais Longe", 10);
							break;
						case 2:
							TextOutA(hdc, sx + 50, sy + 75, "Slice", 5);
							break;
						case 3:
							TextOutA(hdc, sx + 50, sy + 75, "Mouse", 5);
							break;
						case 4:
							TextOutA(hdc, sx + 50, sy + 75, "Random", 6);
							break;
						default:
							TextOutA(hdc, sx + 50, sy + 75, "Personalizado", 13);
							break;
						}
						TextOutA(hdc, sx + 60, sy + 90, (aimbot->modoEspelho ? "Espelho" : "Tornado"), 7);
						TextOutA(hdc, sx + 76, sy + 105, (aimbot->autoShot ? "Sim" : "Nao"), 3);
						if (aimbot->ligado)
							TextOutA(hdc, sx + 60, sy + 120, "Ligado", 6);
						else
							TextOutA(hdc, sx + 60, sy + 120, "Desligado", 9);
...
void __stdcall PASCAL AtualizarValores() {
	...
	__try {
		...
		if (aimbot->enderecos[5] > 0) {
			aimbot->inclinacaoBot = *((PWORD)(aimbot->enderecos[5] + (0x1C * aimbot->indiceBot) + 8));

			for (int i = 0; i < 8; i++) {
				aimbot->semiPosicaoBot[i].x = *((PWORD)(aimbot->enderecos[5] + (0x1C * i)));
				aimbot->semiPosicaoBot[i].y = *((PWORD)(aimbot->enderecos[5] + (0x1C * i) + 4));
			}
		}
		...
		if (aimbot->enderecos[16] > 0)
			aimbot->semiDireita = *((PBYTE)(aimbot->enderecos[16] + (aimbot->indiceBot * 0x1C)));
	...
	while (WaitForSingleObject(base->closeFlag, 6) != WAIT_OBJECT_0) {
		__try{
			...
			__try {
				...
				__except (1) {
					debugar("Falha ao gravar o AutoShot");
				}
