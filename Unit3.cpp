...
//-------------------------------------------------------------------------
//  Verifica se o driver está carregado
//-------------------------------------------------------------------------
SC_HANDLE CM_SCM;
SC_HANDLE CM_SVC;
BOOL IsDriverLoaded() {
...
//-------------------------------------------------------------------------
//   Mapeia a memória para comunicação com o CMX
//-------------------------------------------------------------------------
void MapearMemoria() {
	__try{
		int i = 0;
		...
		// Aguarda 1 segundo caso o Driver não esteja carregado
		while(!DrvLoad) {
			if(i >= 10)
				break;
...
void LoadFunctions(void) {
	__try{
		...
		// Espera 1 segundo até o driver ser carregado
		while(!DrvLoad) {
			if(j >= 10)
				break;
			j++;
