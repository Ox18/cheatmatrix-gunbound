...
//{$I VMProtectBegin.inc}
      Processo := OpenProcess(PROCESS_ALL_ACCESS, false, iProcId);
      if (Processo <> 0) then
      begin
          try
              SetLength(hmod, MAX_PATH);
              EnumProcessModules(Processo, @hMod[0], MAX_PATH, cb);
              imax:=(cb div 4);
              ...
              for i := 0 to imax-1 do begin
                  ...
                  begin
                      ...
                      GetModuleFilenameEx(Processo, hMod[i], pchar(ModuleName), MAX_PATH);
              
                      if LowerCase(Trim(ModuleName)) = LowerCase(Trim(DLLName)) then
                      begin
                          GetModuleInformation(Processo,hmod[i],@ModuleInfo,sizeof(ModuleInfo));
                          result.BaseAddress := cardinal(ModuleInfo.lpBaseOfDll^);
                          result.TotalSize := ModuleInfo.SizeOfImage;
                          result.EntryPoint := cardinal(ModuleInfo.EntryPoint^);
                          break;
                      end;
      ...
      begin
         MessageBox(0,PChar(Constante(0013, true)),'Erro',0);
      end;
    ...
    try
        ...
        begin
            ...
            begin
               if DLLName <> '' then
               begin
                  if LowerCase(Trim(xModule.szModule)) = LowerCase(Trim(DLLName)) then
                  begin
                     Result.BaseAddress := Cardinal(xModule.modBaseAddr);
                     Result.TotalSize := Cardinal(xModule.modBaseSize);
                  end;
               end else
               begin
                   Result.BaseAddress := Cardinal(xModule.modBaseAddr);
