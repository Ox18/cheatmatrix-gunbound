...
Function MD5(valor: AnsiString):AnsiString;
...
//{$I VMProtectBegin.inc}
   result := CM_LowerCase( MD5Print(MD5String(valor)) );
//{$I VMProtectEnd.inc}
end;
...
//{$I VMProtectBegin.inc}
	with Context do begin
		...
		Count[0] := 0;
		Count[1] := 0;
		CM_ZeroMemory(@Buffer, SizeOf(MD5Buffer));
	end;
//{$I VMProtectEnd.inc}
end;
...
//{$I VMProtectBegin.inc} 
	...
	if Length >= PartLen then begin
		CM_CopyMemory(@Context.Buffer[Index], PByte(Input), PartLen);
	...
	CM_CopyMemory(@Context.Buffer[Index], @Input[I], Length - I);
//{$I VMProtectEnd.inc}
end;
...
//{$I VMProtectBegin.inc} 
	...
	CM_ZeroMemory(@Context, SizeOf(MD5Context));
//{$I VMProtectEnd.inc}
end;
...
//{$I VMProtectBegin.inc} 
	MD5Init(Context);
  Path := N;
	FileHandle := CreateFile(pchar(Path), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE,
