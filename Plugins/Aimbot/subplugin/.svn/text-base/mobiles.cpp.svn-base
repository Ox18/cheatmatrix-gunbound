...
int __fastcall TShMatrix::IndexToMobile(int valor) {
	if (valor >= 0 && valor <= 17)
		return valor;
	else if (valor >= 18 && valor <= 27)
		return valor + 1;
	//else if(valor == (quantidadeMobiles-1))
	//	return quantidadeMobiles-2;
	else
		return 0;
}
...
void __fastcall TShMatrix::MobileAnterior() {
	...
	if(j == (quantidadeMobiles-1))
		j = quantidadeMobiles - 2;
	mobile = IndexToMobile(j);
}
...
POINT __fastcall TShMatrix::PosicaoBot() {
	...
	int inclinacaoInicial1 = aimFix.x1 + intVars[0] + maya.velocidadeAngular;
	int raioVirtual1 = aimFix.y1 + intVars[1] + maya.raio;
...
TPointFloatD __fastcall TShMatrix::GetAimFix(int index) {
	...
	case 24: case 25: case 26:
		return TPointFloatD(65, 41);
		break; // Frank
	case 27:
		return TPointFloatD(64, 39);
		break; // Frank
	default:
		break;
	}
...
void __fastcall TShMatrix::fillPoints() {
	__try {
		HDC dc = 0;
...
void __fastcall TShMatrix::desenhaRotacao(TRotacao rotacao, Gdiplus::Pen *caneta, int x, int y, int raio, int comprimentoLinha, int extra) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if ((y + ponto1.y) <= 515 && (y + ponto2.y) <= 515) {
			grafico->DrawLine(caneta, (int)(x + ponto1.x), (int)(y + ponto1.y), (int)(x + ponto2.x), (int)(y + ponto2.y));
		}
		angulo += 90;
	}
}
...
float c1 = 0.0f;
float c2 = 0.0f;
void __fastcall TShMatrix::fillPoints(HDC dc) {
	// VMProtectBegin("fill");
	__try {
		if (!dc)
			return;
...
void __fastcall TShMatrix::desenhaLinha(int indice, bool flag) {
	...
	if ((Projetil[indice].x > (camera.x - 20)) && (Projetil[indice].x < (camera.x + 820)) && (Projetil[indice].y > (camera.y - 20)) && (Projetil[indice].y < (camera.y + 515))) {
		if (indice == 0 && !flag) {
			grafico->DrawLine(linhas[TL_LINHA1].pen, (int)(ultimoProjetil[indice].x - camera.x), (int)(ultimoProjetil[indice].y - camera.y), (int)(Projetil[indice].x - camera.x), (int)(Projetil[indice].y - camera.y));
		}
		else {
			if (ultimoProjetil[indice].x != 0 && ultimoProjetil[indice].y != 0)
				grafico->DrawLine(linhas[TL_LINHA2].pen, (int)(ultimoProjetil[indice].x - camera.x), (int)(ultimoProjetil[indice].y - camera.y), (int)(Projetil[indice].x - camera.x), (int)(Projetil[indice].y - camera.y));
		}
	}
...
void __fastcall TShMatrix::fillPrincess() {
	__try {
		count = 1;
		bool primeiro = true;
...
void __fastcall TShMatrix::fillAssassin() {
	__try {
		count = 1;
		bool primeiro = true;
...
void __fastcall TShMatrix::fillShadowWalker() {
	__try {
		count = 1;
		bool primeiro = true;
...
void __fastcall TShMatrix::fillCarrior() {
	__try {
		count = 1;
		bool primeiro = true;
