...
TMelhorForca __fastcall TCalculador::calcularMelhor(HANDLE closeFlag){
	 ...
	 while( WaitForSingleObject(closeFlag, ((j % 10) == 0)?3:0) != WAIT_OBJECT_0 ){
        ...
        switch(mobile){
			...
			case 21: calculaWolf(); break;
			case 22: calculaTiburon(); break;
			case 23: calculaBlueWhale(); break;
			case 24: calculaFrank(); break;
...
void __fastcall TCalculador::calculaArmor() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaMage() {
	__try {
		count = 1;
	...
	__try {
		...
		for (double t = 0; t <= 50; t += tick){
			...
			if( !flagNak && (int)Projetil[0].y > (int)ultimoProjetil[0].y && Projetil[0].y >= novoY){
				flagNak = true;
				//Projetil[0].speedY *= (1.55+aimbot->teste1.y);
...
void __fastcall TCalculador::calculaRaon() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaIce() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaAduka() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaWolf() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaTiburon() {
	__try {
		count = 1;
...
void __fastcall TCalculador::calculaFrank() {
	__try {
		count = 1;
